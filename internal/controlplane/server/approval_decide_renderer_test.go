...
func TestOrchestrateDecideApprovalHTTP_RegistryParityWithDirectTarget(t *testing.T) {
	...
	request := &approval.Request{ID: "req-http-registry", Decision: approval.DecisionDenied}
	decide := func(*coreapprovalpolicy.DecideApprovalRequest) (*coreapprovalpolicy.ApprovalDecisionResult, error) {
		return &coreapprovalpolicy.ApprovalDecisionResult{Request: request}, nil
	}

	viaRegistry := orchestrateDecideApprovalHTTP(strings.NewReader(body), decide)
