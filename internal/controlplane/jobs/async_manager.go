...
func (m *AsyncManager) CreateForCommand(probeID string, cmd protocol.CommandPayload) (*AsyncJob, error) {
    return m.CreateForCommandInWorkspace(probeID, "", cmd)
}

func (m *AsyncManager) CreateForCommandInWorkspace(probeID, workspaceID string, cmd protocol.CommandPayload) (*AsyncJob, error) {
    job := AsyncJob{
...
func (m *AsyncManager) ListJobsByWorkspace(workspaceID string, limit int) ([]AsyncJob, error) {
    if m == nil || m.store == nil {
        return nil, fmt.Errorf("async manager unavailable")
    }
    return m.store.ListAsyncJobsByWorkspace(workspaceID, limit)
}
...
func (m *AsyncManager) GetJobByRequestID(requestID string) (*AsyncJob, error) {
    if m == nil || m.store == nil {
        return nil, fmt.Errorf("async manager unavailable")
    }
    return m.store.GetAsyncJobByRequestID(requestID)
}
