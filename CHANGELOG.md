...
### Added
- [compat:additive] **E1: Workspace isolation gates for runs/approvals/audit** — Added configurable workspace isolation (`workspace_isolation: true` / `LEGATOR_WORKSPACE_ISOLATION=1`) that scopes all job list/get/run, async job approve/reject, approval queue list/get, audit log query, and command stream replay endpoints to the workspace derived from the auth context (API key `workspace:<id>` claim or session identity). Cross-workspace access returns `403 workspace_forbidden`. Wildcard tokens (`workspace:*`) bypass isolation. Isolation is disabled by default — all existing single-workspace deployments are unaffected. New methods: `ListJobsByWorkspace`, `GetJobCheckWorkspace`, `ListAsyncJobsByWorkspace`, `GetAsyncJobCheckWorkspace`, `PendingByWorkspace`, `AllByWorkspace`, `GetCheckWorkspace`, `SubmitWithWorkspace`. Added `workspace_id` field to `Job`, `AsyncJob`, `JobRun` types, `audit.Event` and `audit.Filter`, and `approval.Request`. Schema migrations are idempotent via `ALTER TABLE ... ADD COLUMN` + `ensureColumn`.
