...
func applyFederationAccessFilterForMCP(filter fleet.FederationFilter, access auth.FederationAccessScope) (fleet.FederationFilter, *federationScopeAuthorizationError) {
...
func actorFromMCPAuthContext(ctx context.Context) string {
...
func (s *MCPServer) recordFederationMCPAuthorizationDenied(ctx context.Context, surface string, requested fleet.FederationFilter, effective fleet.FederationFilter, access auth.FederationAccessScope, authzErr *federationScopeAuthorizationError) {
	...
	s.recordAudit(audit.Event{
		...
		Actor:     actorFromMCPAuthContext(ctx),
		Summary:   fmt.Sprintf("federation authorization denied for %s", surface),
		Detail: map[string]any{
			"surface":               surface,
			"reason":                reason,
			"required_permission":   string(auth.PermFleetRead),
...
func (s *MCPServer) recordFederationMCPReadAudit(ctx context.Context, surface string, requested fleet.FederationFilter, effective fleet.FederationFilter, access auth.FederationAccessScope, sourceCount int, probeCount int) {
	if s == nil || s.auditStore == nil {
		return
	}
...
func (s *MCPServer) recordAudit(evt audit.Event) {
	if s == nil || s.auditStore == nil {
		return
	}
